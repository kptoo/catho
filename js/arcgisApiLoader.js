const ArcGISApiLoader = {
    ...
    async loadCountryBoundaries(countryName) {
        try {
            ...
            if (cached) {
                this.currentCountryBoundaries = cached;
            ...
            this.currentCountryBoundaries = simplified;
    ...
    getAllBoundaries() {
        return this.currentCountryBoundaries ? this.currentCountryBoundaries.features : [];
    },
    
    clearCountry() {
        this.currentCountryBoundaries = null;
        CONFIG.LOADED_COUNTRIES.clear();
    },
    ...
    matchDiocesesToBoundaries(data) {
        ...
        if (!this.currentCountryBoundaries) return matches;
        
        const loadedCountry = Array.from(CONFIG.LOADED_COUNTRIES)[0];
        console.log('Matching dioceses to boundaries for:', loadedCountry);
        
        const countryDioceses = data.filter(d => {
            const dioceseCountry = (d.Country || '').trim();
            return dioceseCountry.toLowerCase() === loadedCountry.toLowerCase();
        });
        
        console.log('Found', countryDioceses.length, 'dioceses in', loadedCountry);
        
        if (countryDioceses.length === 0) {
            console.warn('⚠ No dioceses found for', loadedCountry, '. Check Country field in CSV.');
        }
        
        let matchedCount = 0;
        this.currentCountryBoundaries.features.forEach(boundary => {
    ...
    showLoadingIndicator(countryName) {
        ...
        indicator.innerHTML = `<div style="font-size:18px;font-weight:bold;margin-bottom:15px;">Loading ${countryName}</div><div style="width:300px;height:6px;background:#1a2332;border-radius:3px;overflow:hidden;margin-bottom:10px;"><div style="width:100%;height:100%;background:#3498db;animation:pulse 1.5s ease-in-out infinite;"></div></div><div id="arcgis-loading-status" style="font-size:14px;color:#95a5a6;margin-top:10px;">Connecting to ArcGIS API...</div><style>@keyframes pulse{0%,100%{opacity:1;}50%{opacity:0.5;}}</style>`;
